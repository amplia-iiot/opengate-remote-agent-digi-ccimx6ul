package es.amplia.odm.devices.digi.atcommand;
...
import es.amplia.odm.devices.digi.Utils;
...
import es.amplia.odm.devices.digi.message.data.dmm.Mobile;
import es.amplia.odm.devices.digi.message.data.dmm.Model;
import es.amplia.odm.devices.digi.message.data.dmm.Software;
import es.amplia.odm.devices.digi.message.data.dmm.Subscriber;
...
public class AtCommandManager {
	...
	private static final Pattern CGMI_PATTERN = Pattern.compile(" (.+)  OK");
	...
	private static final Pattern ATI3_PATTERN_SERIAL_NUMBER = Pattern.compile(".*FSN: (\\S+) .*OK");
	...
	public String execute (String command) throws IOException {
		...
		LOGGER.debug("Executed AT Command " + command + ": " + ret.toString());
		return ret.toString();
	}
	...
	public String getMobileCommsModuleModel() {
		String ret;
		try {
			ret = execute("AT+CGMM");
		} catch (IOException e) {
	...
	public String getMobileCommsModuleManufacturer() {
		String ret;
		try {
			ret = execute("AT+CGMI");
		} catch (IOException e) {
	...
	private String getMobileNetworkApn() {
		...
		try {
			...
			logger.log(Level.ERR, "Error getting APN.");
		}
	...
	private String getSignalStrength() {
		...
		if (ret != null) {
			...
			if (rssi != null) return String.valueOf(toDBm(rssi));
		}
		return ret;
	}
	...
	private String getSignailQuality() {
		...
		if (ret != null) {
			...
			if (ber != null) return String.valueOf(berToRate(ber));
		}
		return ret;
	}
	...
	/*private void getCellInfo(Mobile mobile) {
		...
		try {
			...
			logger.log(Level.ERR, "Error executing At Command AT+CCPSI.");
		}
	...
	public String getSerialNumber() {
		...
		try {
			...
			LOGGER.error("Error executing At Command ATI3.", e);
			return null;
		}
		
		if (ret != null) {
			ret = matchResponsePattern(ret, ATI3_PATTERN_SERIAL_NUMBER);
		}
		return ret;
	}
